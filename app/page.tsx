  ...
  <section id={id} className="py-16 sm:py-24">
    <div className="mx-auto max-w-6xl px-4">
      <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} variants={container()} className="mb-10">
        {eyebrow && (<p className="text-xs uppercase tracking-[0.2em] text-gray-500 mb-2">{eyebrow}</p>)}
...
const NavLink = ({ href, children }: any) => (<a href={href} className="text-sm font-medium text-gray-700 hover:text-black transition-colors">{children}</a>);

const Logo = () => (<div className="flex items-center gap-2"><div className="h-8 w-8 rounded-xl bg-gradient-to-br from-indigo-500 via-violet-500 to-fuchsia-500" /><span className="font-extrabold tracking-tight text-xl">Colorstack @ calstala</span></div>);
      ...
      <div className="flex flex-col-reverse sm:flex-row items-center gap-8">
        <motion.div initial="hidden" animate="show" variants={container()} className="flex-1">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <Button asChild href="#join">Join the Chapter <ArrowRight className="ml-2 h-4 w-4 inline"/></Button>
            <Button asChild variant="outline" href="#events">Upcoming Events <CalendarDays className="ml-2 h-4 w-4 inline" /></Button>
          </div>
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-600 flex items-center gap-2"><CalendarDays className="h-4 w-4" /> {ev.date} • <MapPin className="h-4 w-4" /> {ev.where}</p>
                  </div>
                  <Button variant="secondary" size="sm" asChild href="#events">{ev.cta}</Button>
                </div>
              ))}
            </CardContent>
          </Card>
...
const Events = () => (
  <Section id="events" eyebrow="Calendar" title="Upcoming events" subtitle="Come through! Most events are open to all students.">
        ...
        <motion.div key={i} initial="hidden" whileInView="show" viewport={{ once: true }} variants={container(i * 0.05)}>
          <Card className="border-0 shadow-md h-full">
            ...
            <CardContent className="text-sm text-gray-600 space-y-2">
              ...
              <div className="pt-2">
                <Button asChild size="sm" href={ev.link}>RSVP <ExternalLink className="ml-1 h-4 w-4 inline" /></Button>
              </div>
            </CardContent>
          </Card>
        </motion.div>
    ...
    <div className="mt-6 text-sm text-gray-600">Want your event featured? <a href="#contact" className="underline underline-offset-4">Contact us</a>.</div>
  </Section>
);
  ...
  <Section id="sponsors" eyebrow="Partners" title="Sponsors & supporters" subtitle="We partner with companies and campus orgs who share our mission.">
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 items-center">
      {["Acme Tech", "Globex", "Initech", "Umbrella"].map((s, i) => (
        <div key={i} className="aspect-[3/1] rounded-xl border bg-gray-50 flex items-center justify-center text-sm font-medium">
          {s}
        </div>
      ))}
    </div>
    <div className="mt-6">
      <Button asChild variant="outline" href="#contact">Become a sponsor <ChevronRight className="ml-1 h-4 w-4 inline" /></Button>
    </div>
  </Section>
);
  ...
  <Section id="join" eyebrow="Get involved" title="Join the chapter" subtitle="Membership is free. We'll send 1–2 emails a month with events and opportunities.">
    <Card className="border-0 shadow-md">
      <CardContent className="p-6">
        ...
        <p className="mt-3 text-xs text-gray-500">This demo form is non-functional. Hook it up to Google Forms, Airtable, or a serverless action.</p>
      </CardContent>
    </Card>
  </Section>
);
...
const Footer = () => (
  <footer className="py-10 border-t">
    <div className="mx-auto max-w-6xl px-4 flex flex-col sm:flex-row items-center justify-between gap-4">
      <Logo />
      <p className="text-xs text-gray-600">© {new Date().getFullYear()} Colorstack @ calstala. Built with ❤️.</p>
    </div>
  </footer>
);

const Navbar = () => (
  <div className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/70 border-b">
    <div className="mx-auto max-w-6xl px-4 h-16 flex items-center justify-between">
      ...
      <Button asChild size="sm" href="#join">Join</Button>
    </div>
  </div>
);
